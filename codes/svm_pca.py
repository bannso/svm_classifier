...
def getWordVector(featureDir, embeddingDir):
    ...
    num = 0
    failList = []
    record = open("../data/record_pca.txt", "a", encoding="utf-8")
...
def seg_sentence(sentence):
    sentence_seged = jieba.cut(str(sentence).strip())
...
if __name__ == '__main__':
    ...
    myList = ["sgns.target.word-character.char1-2.dynwin5.thr10.neg5.dim300.iter5",
    ...
    for dir in myList:
        ...
        try:
            train(maxFeature=21000, minDf=0, maxDf=1.0, ngram=(1, 1), index=str(myList.index(dir)))
        except:
            print("中断时间:")
            print(time.strftime("%m-%d %H:%M:%S", time.localtime()))
    print("结束时间:")
