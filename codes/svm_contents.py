...
def dataSplit(percent):
    ...
    f = csv.reader(open("../data_qiuyi.cn/contents.csv","r",encoding="utf-8"))
    disease = []
    label = []
    title = []
    text = []
    for line in f:
        ...
        label.append(line[2])
        text.append(line[4])
    data = list(zip(disease, title, label, text))
    random.shuffle(data)
...
def train(maxFeature=None,minDf=0.0,maxDf=1.0,ngram=None,classifier=svm.SVC(kernel='linear')):
    ...
    train.columns = ["disease","title","label","text"]
    test.columns = ["disease","title","label","text"]
# 取出句子的标签
    y_train = train['label']
    y_test = test['label']
# 将训练和测试数据都转成词list
    ...
    for i in range(0, len(train['title'])):
        ...
        # print("train['title'][i]",train["title"][i])
        # print("train['text'][i]",train["text"][i])
        train_data.append(train["title"][i]+str(train["text"][i]))
    test_data = []
    for i in range(0, len(test['title'])):
        # test_data.append(test["title"][i])
        test_data.append(test["title"][i]+str(test["text"][i]))
    ...
    f = open("../data_qiuyi.cn/feature_names_2.txt","w",encoding="utf-8")
...
if __name__ == '__main__':
    ...
    train(maxFeature=None, minDf=0, maxDf=1.0, ngram=(1, 3),classifier=svm.SVC(kernel="rbf",gamma=0.6))
    """
    train(maxFeature=21000, minDf=0, maxDf=1.0, ngram=(1, 3),classifier=svm.SVC(kernel="rbf",gamma=0.6))
    SVM分类模型在测试集上的准确率： 0.9295398977550566
    SVM分类模型在测试集上的P值： [0.95855718 0.9180791  0.87945671 0.96443515]
    SVM分类模型在测试集上的R值： [0.98269079 0.92329545 0.91036907 0.89168279]
    SVM分类模型在测试集上的F1值： [0.97047397 0.92067989 0.89464594 0.92663317]
    """
