...
def getChapter(url):
    ...
    """
    soup = sendRequest(url)
    passages = soup.find(attrs={"class": "articleCont"}).findAll("h2")
    chapters = "".join([passage.get_text().strip() for passage in passages])
    return chapters
...
def getAllContent(dir):
    ...
    """
    fo = csv.reader(open(dir,"r",encoding="utf-8"))
    f = csv.writer(open("../data_120ask/allChapters.csv","w",newline="",encoding="utf-8"))
    fe = csv.writer(open("../data_120ask/errOfChapters.csv","w",newline="",encoding="utf-8"))
    i = ""
    for line in fo:
        try:
            if i != line[1]:
